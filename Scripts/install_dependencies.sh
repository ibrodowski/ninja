...
checkdependencies() {
  ...
  if [ $mySQLInstalled == "5.5.46" ]; then
    ...
    pushd $RFK/
    sudo -H -u $SUDO_USER bash -c 'installer -pkg mysql-5.5.46-osx10.9-x86_64.pkg -target /'
    popd
  ...
  if [ -f $portbinary ]; then
    ...
    echo "MacPorts 2.3.4 is not installed, running installation..."
      pushd $RFK/
      if [ $OS_Version == '10.10.5' ]; then
          sudo -H -u $SUDO_USER bash -c 'installer -pkg MacPorts-2.3.4-10.10-Yosemite.pkg -target /'
          log "MacPorts 2.3.4 is not installed, attempting install for Yosemite..."
      elif [ $OS_Version == '10.11.2' ]; then
          sudo -H -u $SUDO_USER bash -c 'installer -pkg MacPorts-2.3.4-10.11-ElCapitan.pkg -target /'
  ...
  jrebinary="/usr/bin/java"
    if [ -f $jrebinary ]; then
      ...
      sudo -H -u $SUDO_USER bash -c 'installer -pkg OracleJava8-1.8.73.02.pkg -target /'
      popd
    fi
  ...
  jdkbinary="/usr/bin/javac"
    if [ -f $jfkbinary ]; then
      ...
      sudo -H -u $SUDO_USER bash -c 'installer -pkg JDK\ 8\ Update\ 73.pkg -target /'
      popd
    fi
