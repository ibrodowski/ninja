...
log() {
  ...
  sudo -H -u $SUDO_USER bash -c 'mkdir -p "$HOMEDIR"/Documents/DevEnv'
  sudo -H -u $SUDO_USER bash -c 'touch "$HOMEDIR"/Documents/DevEnv/config.log'
...
installdependencies() {
  ...
  if [ -e $RFK ]; then
    ...
    echo "Unable to locate required RFK Software directory under /Library/RFK, please run RFK Software.pkg, prior to running this script..."
    log "Unable to locate required RFK Software directory under /Library/RFK, please run RFK Software.pkg, prior to running this script..."
    exit 3
